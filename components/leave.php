<?php
include "db.php";
// setting minimum dates for date time picker
$today_date = date("Y-m-d"); 
$date_obj = new DateTime($today_date);
$tomorrow_date = $date_obj->modify('+1 day')->format("Y-m-d"); 

// autofilling combo box
$stmt = $conn->prepare("show columns from `leave_requests` like ?");
$stmt->execute(['leave_type']);
$row = $stmt->fetch();
$enum = $row['Type'];

// filtering the string for the enum labels in box
    ...
    <form action="leave.php" method="post">
        <div class="form-context">
            <p><strong>Applicant:</strong>
                <?php isset($_SESSION["firstname"]) ? print htmlspecialchars($_SESSION['firstname']) : print "Guest" ?>
            </p>
            <p>
                <input type="hidden" name="employee_id" id="" value="<?php echo $_SESSION['employeeId'] ?>" readonly>
            </p>
        ...
        <div class="form-group">
            ...
            <select id="leave-type" name="leave-type" required class="form-select">
                ...
                <?php foreach($enum_values as $label):
        ...
        <div class="form-group form-date-group">
            <div class="date-input-wrapper">
                ...
                <input type="date" id="from-date" name="from-date" required class="form-input" min="<?= htmlspecialchars($today_date)?>">
            </div>
            <div class="date-input-wrapper">
                <label for="to-date">To Date <span class="required-star">*</span></label>
                <input type="date" id="to-date" name="to-date" required class="form-input" min="<?= htmlspecialchars($tomorrow_date)?>">
            </div>
        </div>
